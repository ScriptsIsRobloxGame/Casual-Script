...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Интерфейс скрыт", Content = "Интерфейс был скрыт, вы можете показать его снова, нажав 'Show'.", Duration = 7, Image = 4400697855})
		else
			RayfieldLibrary:Notify({Title = "Интерфейс скрыт", Content = `Интерфейс был скрыт, вы можете показать его снова, нажав {getSetting("General", "rayfieldOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
function RayfieldLibrary:CreateWindow(Settings)
    if Rayfield:FindFirstChild('Loading') then
        if getgenv and not getgenv().rayfieldCached then
            ...
            label.Parent = Rayfield:FindFirstChild("Loading")
            label.Text = "Casual Script"
            label.Size = UDim2.new(1.155, 0, 1.4, 0)
            ...
            Rayfield.Loading.Banner.Visible = false		
		...
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					Title = "Casual Script",
					Content = "Тебе нравится наш скрипт? Тогда присоединяйся к нам в Telegram: @casualscript",
					Duration = 7,
					Image = 4370033185,
				})
			end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						...
						RayfieldLibrary:Notify({Title = "Система ключей", Content = "Ключ для этого скрипта был успешно сохранён.", Image = 3605522284})
					end
				else
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "Rayfield Конфигурация", Content = "Мы не смогли включить сохранение конфигурации, поскольку вы не используете программное обеспечение с поддержкой файловых систем.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "Rayfield Конфигурация", Content = "Файл конфигурации для этого скрипта был загружен из предыдущей сессии.", Image = 4384403532})
		elseif not success and not notified then
			warn('Rayfield Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "Rayfield Конфигурация", Content = "Произошла ошибка при загрузке вашей конфигурации.\n\nПроверьте консоль разработчика для получения дополнительной информации.", Image = 4384402990})
		end
	end
